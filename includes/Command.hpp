...
class Command {
	...
	public :
		...
		void cmd_quit_closed(Server& server, int c_fd);
		void cmd_part_closed(Server& server, int c_fd);
		...
		const std::string ERR_CHANNELISFULL(Server& server, int c_fd, std::string ch_name); // 471
		...
		const std::string ERR_KEYSET(Server& server, int c_fd, Channel* channel); // 467
